...
function Installation({ data }) {
  // console.log(
  //   "pages - installations - [slug].js - getStaticProps - props: ",
  //   data
  // );
...
export async function getStaticPaths() {
  const { data } = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_API}/installations`
  );
...
export async function getStaticProps({ params }) {
  const { data, notFound } = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_API}/installations?publicationState=live&populate[seo][populate]=%2A&populate[pageHeading][populate]=%2A&populate[openGraph][populate]=%2A&populate[block][populate]=%2A&filters[slug]=${params.slug}`
  );
